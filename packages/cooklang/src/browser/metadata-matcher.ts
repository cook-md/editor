...
// `searchRecipes`'s `fields`/`where` digest (this package) and
// `updateRecipeMetadata`'s selector (`packages/cooklang-ai`). The two
// packages do not depend on each other (see `batch-args.ts` for the
// precedent), so this file is duplicated verbatim at
// `packages/cooklang-ai/src/browser/file-tools/metadata-matcher.ts`
