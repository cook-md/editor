...
- **Closed during implementation:** servings / total time / difficulty now come from each recipe's Cooklang metadata (GitHub and feeds; difficulty limited to easy|medium|hard by the DB constraint), and feed recipes now store ingredients, so all filters cover both sources. The one-time rebuild backfills both.
- **API errors are always JSON** (`{"error": …}`), including malformed query strings and 429 (with `Retry-After`).
- **Plugin state:** Theia's plugin `workspaceState`/`globalState` was only flushed every 60–70 s and lost on quit; fixed in `@theia/plugin-ext` (flush on backend stop). The panel's filters persist per workspace via `workspaceState`.
- **Known limitations:** a read-only source tab for a `cooklang-hub:` recipe is not restored after restart (Theia's 3 s plugin-file-system activation timeout; the preview itself recovers); changing `recipeHub.serverUrl` does not retarget previews already open; the homepage "Recently Indexed" list is empty because `recipes.indexed_at` is never set (pre-existing).
